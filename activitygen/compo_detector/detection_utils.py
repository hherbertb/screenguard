...
def is_block(clip, thread=0.15):
    ...
    blank_count = 0
    for i in range(1, 5):
        if np.sum(clip[side + i]) / 255 > thread * clip.shape[1]:
            blank_count += 1
    if blank_count > 2:
        return False
    ...
    blank_count = 0
    for i in range(1, 5):
        if np.sum(clip[:, side + i]) / 255 > thread * clip.shape[0]:
            blank_count += 1
    if blank_count > 2:
        return False
...
def nested_components_detection(
    ...
    if show:
        plt.imshow(cv2.cvtColor(broad_all, cv2.COLOR_BGR2RGB))
        plt.title("flood-fill all")
        plt.show()
        plt.imshow(cv2.cvtColor(broad, cv2.COLOR_BGR2RGB))
        plt.title("block")
        plt.show()
